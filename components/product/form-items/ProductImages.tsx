...
export function ProductImage({
	...
	const imageUrl = useMemo(() => URL.createObjectURL(src), [src]);

	return (
		<div className='grid gap-2'>
			<div
				className={cn(
					'relative group rounded-md border border-border overflow-hidden',
			...
			{errorMessage && (
				<p className='text-destructive'>{errorMessage.message}</p>
			)}
		</div>
	);
}
